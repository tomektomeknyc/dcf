...
###################################  CHANGE 1 #######################################
# Track changes in selected stocks to reset LLM/statistical flags
if "last_selected_stocks" not in st.session_state:
    st.session_state["last_selected_stocks"] = []

if st.session_state["last_selected_stocks"] != selected_stocks:
    # Stock selection has changed → reset analysis flags
    for _key in ("ff5_ran", "capm_ran", "damo_ran", "intrinsic_computed"):
        st.session_state[_key] = False
    st.session_state["last_selected_stocks"] = selected_stocks.copy()
...
for _key in ("ff5_ran", "capm_ran", "damo_ran"): #----> Remove it (1)
    if _key not in st.session_state: # ------> and this...(2)
        st.session_state[_key] = False #------> Since it will be handled dynamically (3)
